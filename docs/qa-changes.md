...
The runner's env files — `GITHUB_ENV`, `GITHUB_PATH`, `GITHUB_OUTPUT` and
`GITHUB_STEP_SUMMARY` — are removed for the same reason. They are not data: a line appended to
`GITHUB_ENV` or `GITHUB_PATH` is applied by the runner to every step that comes *after* QA in
the calling job, which would let the branch under test set variables, or put a directory on
`PATH`, for steps it should have no say in. The agent reaches the verdict and the job summary
through the action itself, so its commands never need them.
